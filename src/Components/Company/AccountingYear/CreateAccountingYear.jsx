        ...
        setFormData(prevState => {
            ...
            if (name === "Start_Date") {
                ...
                if (!isNaN(startDate.getTime())) {  // Check if the date is valid
                    ...
                    // Determine if the date is April 1 or later
                    if (startDate.getMonth() > 2 || (startDate.getMonth() === 2 && startDate.getDate() > 31)) {
                        // If April 1 or later, set end date to March 31 of the following year
                        endDate = new Date(startDate.getFullYear() + 1, 2, 31, 15); // Adding 15 hours to ensure it rounds to the correct day
                    } else {
                        // If before April 1, set end date to March 31 of the same year
                        endDate = new Date(startDate.getFullYear(), 2, 31, 15); // Adding 15 hours to ensure it rounds to the correct day
                    }
    
                    // Format the end date and update the End_Date field in formData
                    updatedFormData['End_Date'] = endDate.toISOString().split('T')[0];
    
                    // Calculate and update the fiscal year string
                    const startYear = startDate.getFullYear();
                    const endYear = endDate.getFullYear();
                    updatedFormData.year = `${startYear}-${endYear}`;
                } else {
                    // Handle invalid start date by clearing the end date and year fields
    ...
    const fetchAccountingYear = () => {
        fetch(`${API_URL}/get_latest_accounting_year?Company_Code=${companyCode}`)
            ...
            .then(data => {
                ...
                // No yearCode found, default to 1
                setFormData(prevState => ({
                    ...prevState,
                    yearCode: 1
                }));
