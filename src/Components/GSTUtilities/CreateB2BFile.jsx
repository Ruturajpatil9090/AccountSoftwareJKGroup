...
const CreateB2BFile = ({ fromDate, toDate, companyCode, yearCode }) => {
    ...
    const fetchHSNWiseSummary = async () => {
        try {
            setLoading(true);
            setError('');
            const response = await axios.get(`${API_URL}/CreateB2BFileData-summary`, {
                params: {
                    from_date: fromDate,
                    to_date: toDate,
    ...
    const openReportInNewTab = (data) => {
        ...
        newWindow.document.write(`
            <html>
                <head>
                    <title>Create B2B File Report</title>
                ...
                <body>
                    ...
                    <script>
                        window.exportToXlsx = function() {
                            ...
                            const columnOrder = [ 'GSTIN/UIN of Recipient','Invoice Number','Invoice date','Invoice Value','Place Of Supply','Reverse Charge','Invoice Type','E-Commerce GSTIN','Rate','Taxable Value','Cess Amount'];
                            const ws = XLSX.utils.json_to_sheet(data, { header: columnOrder, skipHeader: false });
                            const wb = XLSX.utils.book_new();
                            XLSX.utils.book_append_sheet(wb, ws, 'B2B Report');
                            XLSX.writeFile(wb, 'CreateB2BFile.xlsx');
                        };
                    </script>
                </body>
            </html>
